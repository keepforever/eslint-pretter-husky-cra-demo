        ...
        <tbody>
            {subs.map((sub) => {
                const cart = carts.find(
                    (cart) => cart.sop_id === sub.internal_id
                );
                if (!cart) return null;
                return (
...
const SopWidget = ({
    ...
    const quarterly = sop.filter(
        (sub) =>
            sub.frequency === 'quarterly' && sub.soptype !== 'recorded books'
    );
        ...
        <div id="SOP-widget" className="dashboard-widget">
            ...
            {!hasSOP && (
                <>
                    ...
                    <p>
                        <Link to="/sop">
                            Learn more about how our Standing Order Plans can
                            help you keep the freshest content on your shelves
                        </Link>
                        .
                    </p>
                </>
            ...
            {hasMonthly && type === 'monthly' && (
                <div className="tab-content-slim" id="carts">
                    <h4 className="mt-2">
                        ...
                        <SopStatus
                            sub={monthly[0]}
                            programOpen={sopState.is_open}
                            isOpen={sopState.is_open}
                        />
                    </h4>
                    <SopTable
                        carts={carts}
