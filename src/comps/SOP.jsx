    ...
    <table className="table cart-table carts-table table-slim">
        ...
        <tbody>
            {subs.map(sub => {
                ...
                if (!cart) return null;
                return <tr key={sub.internal_id}>
                    <td><Link to={`${cartUtils.getUrl(cart, accountId)}?ref=dashboard`}>{cartUtils.getName(cart, sub)}</Link></td>
...
const SopWidget = ({account_id, org_id, org_currency, sop, carts, price_contracts, sopState, location}) => {
    ...
    useEffect(() => {
        const scroll = (location.hash === '#sop' || location.query.ref === 'sop') && window.localStorage.getItem('hide-dashboard-sop-modal');
        if (scroll) {
    ...
    return (
        <div id="SOP-widget" className="dashboard-widget">
            ...
            {!hasSOP && <>
                ...
                <p><Link to="/sop">Learn more about how our Standing Order Plans can help you keep the freshest content on your shelves</Link>.</p>
            </>}

            {hasSOP && <div className="tabs tabs-slim">
                {hasMonthly && <a href="#monthly" className={classnames({active: type === 'monthly'})} onClick={(e) => {e.preventDefault(); setType('monthly')}}>Monthly</a>}
                {hasQuarterly && <a href="#quarterly" className={classnames({active: type === 'quarterly'})} onClick={(e) => {e.preventDefault(); setType('quarterly')}}>Quarterly</a>}
                {hasRb && <a href="#rb" className={classnames({active: type === 'rb'})} onClick={(e) => {e.preventDefault(); setType('rb')}}>RB</a>}
            </div>}

            {hasMonthly && type === 'monthly' && <div className="tab-content-slim" id="carts">
                <h4 className="mt-2">Currently <SopStatus sub={monthly[0]} programOpen={sopState.is_open} isOpen={sopState.is_open} /></h4>
                <SopTable carts={carts} subs={monthly} currency={org_currency} priceContracts={price_contracts} />
            </div>}

            {hasQuarterly && type === 'quarterly' && <div className="tab-content-slim" id="quarterly">
                <h4 className="mt-2">Currently <SopStatus sub={quarterly[0]} programOpen={sopState.is_open} isOpen={sopState.is_open} /></h4>
                <SopTable carts={carts} subs={quarterly} currency={org_currency} priceContracts={price_contracts} accountId={account_id} />
            </div>}

            {hasRb && type === 'rb' && <div className="tab-content-slim" id="rb">
                <h4 className="mt-2">Currently <SopStatus sub={rb[0]} programOpen={sopState.is_open} isOpen={sopState.is_open} /></h4>
                <SopTable carts={carts} subs={rb} currency={org_currency} priceContracts={price_contracts} accountId={account_id} />
            </div>}
        </div>
    )
};
